...
import {
    BestmedRiskResolverAsync,
    BestmedProcedureRepositoryAsync,
} from '@/lib/risk/bestmed-resolver-async';
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
    ...
    try {
        const audit = await BestmedRiskResolverAsync.resolve(planSlug, procedureSlug);
        const cost = new Intl.NumberFormat('en-ZA', { style: 'currency', currency: 'ZAR', maximumFractionDigits: 0 }).format(audit.liability);

        const canonicalUrl = `https://www.intellihealth.co.za/bestmed/${planSlug}/${procedureSlug}`;
        const title = `${audit.procedure.label} Cost on ${audit.plan.plan_name} | ${cost} Co-payment`;
        const description = `Calculate your ${audit.procedure.label} hospital co-payment on Bestmed ${audit.plan.plan_name}. 2026 liability: ${cost}. Network rules and PMB exemptions.`;
        ...
        return { title: 'Procedure Cost Calculator | Bestmed' };
    }
}

export default async function BestmedProcedureLeafPage({ params }: PageProps) {
    const { planSlug, procedureSlug } = await params;

    let audit;
    try {
        audit = await BestmedRiskResolverAsync.resolve(planSlug, procedureSlug);
    } catch {
        notFound();
    }
    ...
    const faqs = [
        ...
        },
        {
            ...
            answer: `Voluntary use of a non-network hospital triggers an additional ${formatCost(audit.breakdown.deductibles.non_network_penalty)} penalty on top of procedure co-payments.`
        },
        {
            question: `Is a ${audit.procedure.label} covered as a PMB?`,
            answer: `If your condition qualifies as a Prescribed Minimum Benefit (PMB), the co-payment may be waived. Check with Bestmed to confirm your diagnosis qualifies.`
        },
    ];
            ...
            <main className="min-h-screen bg-slate-50 pb-20">
                ...
                <section className="max-w-3xl mx-auto pt-8 px-4 space-y-6">
                    ...
                    <div className="bg-white rounded-[2rem] p-8 shadow-lg border border-slate-200 relative overflow-hidden">
                        <div className="absolute top-0 right-0 w-40 h-40 bg-blue-50 rounded-full blur-3xl opacity-60 -translate-y-1/2 translate-x-1/2" />
                        ...
                        <div className="relative z-10">
                            ...
                            {audit.liability > 0 && (
                                <div className="mt-6 space-y-3">
                                    <h4 className="text-sm font-bold text-slate-400 uppercase tracking-wider">Breakdown</h4>
                                    <div className="grid gap-2">
                                        {audit.breakdown.deductibles.procedure_copayment > 0 && (
                                            <div className="flex justify-between p-3 bg-slate-50 rounded-xl">
                                                <span className="text-slate-600">Procedure co-payment</span>
                                                <span className="font-bold text-slate-800">
                                                    {formatCost(audit.breakdown.deductibles.procedure_copayment)}
                                                </span>
                                            </div>
                                        )}
                                        {audit.breakdown.deductibles.day_hospital_copayment > 0 && (
                                            <div className="flex justify-between p-3 bg-slate-50 rounded-xl">
                                                <span className="text-slate-600">Acute hospital surcharge</span>
                                                <span className="font-bold text-slate-800">
                                                    {formatCost(audit.breakdown.deductibles.day_hospital_copayment)}
                                                </span>
                                            </div>
                                        )}
                                    </div>
                                ...
                                <div className="mt-6 pt-6 border-t border-slate-100">
                                    ...
                                    <div className="grid grid-cols-2 gap-3">
                                        <div className="p-4 bg-emerald-50 rounded-xl text-center">
                                            <p className="text-xs text-emerald-600 font-medium mb-1">DSP Day Hospital</p>
                                            <p className="text-xl font-bold text-emerald-700">
                                                {formatCost(audit.breakdown.variants.day_hospital_dsp)}
                                            </p>
                                        </div>
                                        <div className="p-4 bg-amber-50 rounded-xl text-center">
                                            <p className="text-xs text-amber-600 font-medium mb-1">Acute Hospital</p>
                                            <p className="text-xl font-bold text-amber-700">
                                                {formatCost(audit.breakdown.variants.day_hospital_acute)}
                                            </p>
                                        </div>
                                    </div>
                                </div>
                    ...
                    <div className="bg-red-50 rounded-2xl p-5 border border-red-100 flex gap-4">
                        ...
                        <div>
                            ...
                            <p className="text-sm text-red-700/80 mt-1">
                                ...
                                <span className="font-black"> {formatCost(audit.breakdown.deductibles.non_network_penalty)} </span>
                                to your total.
                            </p>
                        </div>
                    </div>
                        ...
                        <div className="grid md:grid-cols-2 gap-3">
                            {relatedProcedures.map(proc => (
                                <Link
                                    ...
                                    className="group flex items-center justify-between p-3 bg-slate-50 hover:bg-blue-50 rounded-xl border border-slate-100 hover:border-blue-200 transition-all"
                                >
                                    <span className="font-medium text-slate-600 group-hover:text-blue-700 text-sm">
                                        {proc.label}
                                    </span>
